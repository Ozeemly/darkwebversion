...
@section('content')
    <div class="gs-vendor-outlet">
        ...
        <div class="vendor-edit-profile-section-wrapper">
            <div class="gs-edit-profile-section">
                ...
                <form class="edit-profile-area" action="{{route('vendor-pickup-point-update',$data->id)}}" method="POST">
                    @csrf
                    <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="location" class="form-control" placeholder="@lang('Click on map to select location')" 
                                   name="location" value="{{$data->location}}" readonly>
                            @error('location')
                        ...
                        <div class="col-12 col-sm-12">
                            <button class="template-btn btn-forms" type="submit" id="submit-btn">
                                @lang('Update')
                            </button>
                        </div>
                    </div>
    ...
    <script>
        ...
        const existingLatitude = {{ $data->latitude ?? 'null' }};
        const existingLongitude = {{ $data->longitude ?? 'null' }};
        const existingLocation = @json($data->location ?? '');

        function initMap() {
            // Use existing coordinates if available, otherwise use default location
            let initialLocation;
            if (existingLatitude && existingLongitude) {
                initialLocation = { lat: parseFloat(existingLatitude), lng: parseFloat(existingLongitude) };
            } else {
                initialLocation = { lat: 23.8103, lng: 90.4125 }; // Default location (Dhaka, Bangladesh)
            }
            ...
            if (existingLatitude && existingLongitude) {
                ...
                // Try to get user's current location if no existing coordinates
                getCurrentLocation();
            }
            ...
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(
                    ...
                    (error) => {
                        ...
                        // If geolocation fails and no existing coordinates, keep default location
                    },
                    {
